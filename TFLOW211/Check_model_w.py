...
def drawtangle(box,cls,cimg):
	...
	cv2.putText(cimg,str(int(cls)),(cx,cy),cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,255),2)
...
export_dir = './mydata/exported_model_1600_fcw'
...
data_root = pathlib.Path('./mydata/check_picw')
	...
	if ('.JPG' in fn.upper() or '.JPEG' in fn.upper() or '.PNG' in fn.upper() or '.BMP' in fn.upper()):
		...
		for i in range(100):
			if float(output_dict['detection_scores'][0][i]) >= score:
				drawtangle(output_dict['detection_boxes'][0][i],output_dict['detection_classes'][0][i],cimg)
